...
def doc2vec(documents):
    ...
    texts = [[word for word in doc.lemma_.split() if word not in stop_list]
    ...
    texts = [[token for token in snipp if frequency[token] > 1]
             for snipp in texts]
...
def similariy(corpus_lsidf, query_lsidf):
    index = gensim.similarities.SparseMatrixSimilarity(
        corpus_lsidf, num_features=100000)
...
def get_candidate_answers(question_query, ranked_docs, en_nlp):
    ...
    combined_document = []
    for _doc in ranked_docs:
        combined_document.append(_doc.get_verse())
